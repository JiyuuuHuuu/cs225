...
T sum(stack<T>& s)
{
    ...
    retVal = temp + sum(s);
    s.push(temp);
    return retVal; // stub return value (0 for primitive types). Change this!
...
template <typename T>
void scrambleHelp(queue<T>& q, int n)
{
    if (q.size() == 0)
      return;
...
bool verifySame(stack<T>& s, queue<T>& q)
{
    ...
    bool lastRet = verifySame<T>(s, q);
    T temp_q = q.front();
    q.pop();
    retVal = (temp_q == temp_s) && lastRet;
    q.push(temp_q);
    s.push(temp_s);
    return retVal;
}
