...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     auto temp = frequencies.begin();
     while (temp != frequencies.end())
     {
     ...
     {
       // cout<<"1"<<endl;
       min1 = removeSmallest(singleQueue, mergeQueue);
       min2 = removeSmallest(singleQueue, mergeQueue);
       newNode = new TreeNode((min1->freq).getFrequency()+(min2->freq).getFrequency());
       newNode->left = min1;
       newNode->right = min2;
       mergeQueue.push(newNode);
     }
     newNode = mergeQueue.front();
     root_ = newNode;
}
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
     if (current->left == NULL && current->right == NULL)
     {
        ...
        // cout<<"1"<<(current->freq).getCharacter();
        return;
     }
     bfile.writeBit(0);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if (bfile.getNextBit())
     {
       ret = new TreeNode(Frequency(bfile.getNextByte(),0));
     }
     else
     {
       ret = new TreeNode(0);
