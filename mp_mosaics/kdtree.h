...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> findKth(const vector<Point<Dim>>& newPoints, int k, int dim);

    void constHelper(const vector<Point<Dim>>& newPoints, KDTreeNode *& subroot, int dim);

    void copyHelper(KDTreeNode *& subroot, const KDTreeNode * other);

    void destroy(KDTreeNode * subroot);

    Point<Dim> findHelper(const Point<Dim>& query, const KDTreeNode* subroot, int dim) const;

    double getDistance(const Point<Dim>& first, const Point<Dim>& second) const;



    // void leftsubTreeTraversal(KDTreeNode* subroot, Point<Dim>& currentBest,
    //   const Point<Dim>& query, double& radius, int level) const;
    //
    // void darkMagic(KDTreeNode* subroot, Point<Dim>& currentBest, int& found,
    //   const Point<Dim>& query, double& radius, int level, int& exactMatch) const;
    //
    // void subTreeTraversal(KDTreeNode* subroot, Point<Dim>& currentBest,
    //   const Point<Dim>& query, double& radius, int level) const;
};
