...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
  ...
  while (table[index]!= NULL) {
    i++;
    index = (index + i*(hashes::secondary_hash(key,size)))%size;
  }
...
{
  int remove_ele = findIndex(key);
  if(remove_ele != -1){
    delete table[remove_ele];
    table[remove_ele] = NULL;
    --elems;
  }
    /**
     * @todo Implement this function
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
  ...
  while(should_probe[i]){
    ...
    i = (i + j*(hashes::secondary_hash(key,size)))%size;
    if(first == i) break;
  }
    /**
     * @todo Implement this function
